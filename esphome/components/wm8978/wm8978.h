...
/* ---------- WM8978.cpp (https://github.com/CelliesProjects/wm8978-esp32) ---------- */
...
static const char *TAG = "custom-wm8978";
...
uint8_t WM8978::Write_Reg(uint8_t reg, uint16_t val)
{
  ...
  #endif  // USE_ARDUINO
  #ifdef USE_ESP_IDF
  esp_err_t err = i2c_master_write_to_device(I2C_NUM_0, WM8978_ADDR, (const uint8_t*)buf, 2, 1000 / portTICK_RATE_MS);
  #endif // USE_ESP_IDF
...
void WM8978::setALC(uint8_t enable, uint8_t maxgain, uint8_t mingain)
{
  ...
  uint8_t level = 0b1011;
  uint8_t hold = 0b0010;
  regval = (hold<<4) | (level<<0);
  Write_Reg(33, regval);

  uint8_t mode = 0b0;
  uint8_t decay = 0b0011; // ramp-up
  uint8_t attack = 0b0010; // ramp-down
  regval = (mode<<8) | (decay<<4) | (attack<<0);
  Write_Reg(34, regval);
}
...
//      .master.clk_speed = frequency,
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "I2C setup error sda=%i scl=%i frequency=%i", sda, scl, frequency);
    return false;
  }
  #endif // USE_ESP_IDF
